...
```js
1.先在本地启用http-server,在文件当前位置输入以下命令http-server . -c-1

2.然后再index.html引入ts文件的时候，会报错，输入以下命令即可tsc -w xxx.ts,这个命令会不断地把ts转换成js

3.详细代码在calculator文件夹下
```
...
```js
1. keys:Array<string>表示keys是一个字符串数组
2.Array<Array>表示数组里面还是数组
```
